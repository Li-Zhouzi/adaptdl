...
def create_job_infos_from_global_state(jobs, global_profile_path="checkpoint/global-profile-state"):
    ...
    for i, (job_key, job_resources) in enumerate(jobs.items()):
        ...
        # Skip if application not in APPLICATIONS config
        if app_name not in APPLICATIONS:
            skipped_jobs.append((job_str, f"App {app_name} not in APPLICATIONS config"))
            continue

        # Get parameters for this application
        if app_name not in perf_params_dict:
            skipped_jobs.append((job_str, f"No perf_params for {app_name}"))
            continue
        ...
        if app_name not in grad_params_dict:
            skipped_jobs.append((job_str, f"No grad_params for {app_name}"))
            continue

        app_grad_params = grad_params_dict[app_name]
        if 0 not in app_grad_params:
            skipped_jobs.append((job_str, f"No grad_params for epoch 0 in {app_name}"))
            continue
        ...
        job_infos[job_str] = JobInfo(
            ...
            resources=resources,
            min_replicas=1,
            max_replicas=min(64, max(4, 8)),  # Fixed reasonable max
            preemptible=True,
    ...
    if skipped_jobs:
        print(f"Skipped {len(skipped_jobs)} jobs:")
        for job, reason in skipped_jobs[:5]:  # Show first 5
            print(f"  {job}: {reason}")
        if len(skipped_jobs) > 5:
            print(f"  ... and {len(skipped_jobs) - 5} more")
